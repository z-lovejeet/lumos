...
export async function GET(req: Request) {
  ...
  try {
    const plan = await prisma.careerPlan.findFirst({
      where: { userId: user.id, type: 'ms-abroad' }
    });
    return NextResponse.json({ plan });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to fetch MS Abroad plan' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to save MS Abroad plan' }, { status: 500 });
  }
}
